# -*- coding: utf-8 -*-
"""
Created on Tue Nov 21 16:26:34 2023
...
auto=pd.read_csv("C:/CSV files/autoInsurance.csv")

a=auto.describe()
#column state will not affect the data so drop it
auto.dtypes
autonew=auto[['Customer Lifetime Value','Income','Monthly Premium Auto','Months Since Last Claim','Months Since Policy Inception','Number of Open Complaints','Number of Policies','Total Claim Amount']]
...
df_norm=norm_func(autonew.iloc[:,1:])
...
autonew["clust"]=mb
autonew.columns
autonew=autonew.iloc[:,[8,0,1,2,3,4,5,6,7]]
autonew
autonew.iloc[:,2:8].groupby(autonew.clust).mean()
autonew.to_csv("kmeans_auto.csv",encoding="utf-8")
import os
os.getcwd()
