...
df_norm.drop(["Quarter_Q3"],axis=1,inplace=True)
...
df_norm["clust"]=mb
df_norm.columns
df_norm.drop(["clust"],axis=1,inplace=True)#dropping bcoz we will 
df_norm.insert(0,"clust",mb)#use other method to reset index
df_norm
df_norm.iloc[:,2:8].groupby(df_norm.clust).mean()
df_norm.to_csv("kmeans_tele_cus.csv",encoding="utf-8")
import os
os.getcwd()
